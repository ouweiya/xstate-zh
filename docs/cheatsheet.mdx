...
使用此备忘单快速查找 XState v5 的语法。

## Installing XState (安装 XState)
...
## Creating a state machine (创建状态机)
...
[了解更多关于角色模型](actor-model.mdx)。

## Creating promise logic (创建 Promise 逻辑)
...
[了解更多关于Promise角色逻辑](/docs/actors#actors-as-promises)。

## Creating transition logic (创建转换逻辑)
...
[了解更多关于转换器角色](/docs/actors#fromtransition)。

## Creating observable logic (创建可观察逻辑)
...
[了解更多关于可观察器角色](/docs/actors#fromobservable)。

## Creating callback logic (创建回调逻辑)
...
[了解更多关于回调器角色](/docs/actors#fromcallback)。
...
[了解更多关于调用角色](invoke.mdx)。

## Spawning actors (生成角色)
...
## Invoking actors with input (带输入的角色调用)
...
[了解更多关于带输入的角色调用](input.mdx#invoking-actors-with-input)。
